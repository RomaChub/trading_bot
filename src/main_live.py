    ...
    #                     error_msg = str(e).lower()
    #                     if "cannot schedule new futures after shutdown" in error_msg or "shutdown" in error_msg:
    #                         print(f"[{symbol}] ⚠️ Executor was shut down, stopping monitoring")
    #                         break
    #                     print(f"[{symbol}] ⚠️ RuntimeError submitting ticker request: {e}")
    ...
    #                     print(f"[{symbol}] ⚠️ Executor was shut down, stopping monitoring")
    #                     break
    #                 print(f"[{symbol}] ⚠️ RuntimeError submitting positions request: {e}")
    #                 time.sleep(args.update_interval)
    #                 continue
    #
    #             try:
    ...
    #                     error_msg = str(e).lower()
    #                     if "cannot schedule new futures after shutdown" in error_msg or "shutdown" in error_msg:
    #                         print(f"[{symbol}] ⚠️ Executor was shut down, skipping cleanup")
    #                     else:
    #                         print(f"[{symbol}] ⚠️ RuntimeError submitting cleanup: {e}")
    #                 if live_chart:
    ...
    #                                 error_msg = str(e).lower()
    #                                 if "cannot schedule new futures after shutdown" in error_msg or "shutdown" in error_msg:
    #                                     print(f"[{symbol}] ⚠️ Executor was shut down, skipping trades fetch")
    #                                     recent_trades = []
    #                                 else:
    #                                     print(f"[{symbol}] ⚠️ RuntimeError submitting trades request: {e}")
    #                                     recent_trades = []
    #                             else:
    #                                 try:
    #                                     recent_trades = trades_future.result(timeout=5)
    #                                 except FutureTimeoutError:
    #                                     print(f"[{symbol}] ⚠️ Timeout getting recent trades")
    #                                     recent_trades = []
    #
    ...
    #                     print(f"[{symbol}] ⚠️ Executor was shut down, stopping monitoring")
    #                     break
    #                 print(f"[{symbol}] ⚠️ RuntimeError submitting klines request: {e}")
    #                 time.sleep(args.update_interval)
    #                 continue
    #
    #             try:
